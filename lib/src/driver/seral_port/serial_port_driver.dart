  ...
  Future<ToolReturn> call(FunctionCall functionCall) async {
    try {
      if (functionCall.name == "getAvailablePorts") {
        ...
        String name = functionCall.arguments["name"];
        int baudRate = functionCall.arguments["baudRate"] ?? 9600;
        int bits = functionCall.arguments["bits"] ?? 8;
        String parity = functionCall.arguments["parity"] ?? "none";
        int stopBits = functionCall.arguments["stopBits"] ?? 1;
        int rts = functionCall.arguments["rts"] ?? 1;
        int cts = functionCall.arguments["cts"] ?? 0;
        int dtr = functionCall.arguments["dtr"] ?? 1;
        int dsr = functionCall.arguments["dsr"] ?? 1;
        int xonXoff = functionCall.arguments["xonXoff"] ?? 0;
        ...
        String command = functionCall.arguments["command"];
        int timeout = functionCall.arguments["timeout"] ?? 1000;
        ...
        int bufferBytes = functionCall.arguments["bufferBytes"] ?? 10000;
        int timeout = functionCall.arguments["timeout"] ?? -1;
